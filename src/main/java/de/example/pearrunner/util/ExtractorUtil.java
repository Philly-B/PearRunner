...
public class ExtractorUtil {
	...
	public List<AnnotationDto> extractAnnotations(JCas cas) {
		...
		for (Type annotationType : annotationTypes) {
			...
			for (Annotation annotation : annotationIndex) {
				// filter subtypes to avoid duplicate return of subtypes (i.e. do not return DiagnosisConcept if annotationType is Concept)
	...
	private AnnotationDto buildAnnotationDtoRecursive(JCas jCas, Annotation annotation, Set<Integer> parentAdresses) {
		...
		ExtractorUtil.log.error("Ignoring annotation of type [{}] with illegal offsets. Begin: [{}], end: [{}], document text length: [{}]",
				annotation.getType(),
				String.valueOf(annotation.getBegin()),
				String.valueOf(annotation.getEnd()),
	...
	private boolean areOffsetsValid(JCas jCas, Annotation annotation) {
		...
		return annotation.getBegin() >= 0 && annotation.getBegin() <= annotation.getEnd() && annotation.getEnd() <= jCas.getDocumentText().length();
	}


	private Object buildFeatureValueObject(JCas jCas, Annotation annotation, Feature feature, Set<Integer> parentAdresses) {
	...
	private Object buildFeatureValueFromArray(JCas jCas, Annotation annotation, Feature feature, Set<Integer> parentAdresses) {
	...
	private Object buildFeatureValueFromComplexArray(JCas jCas, Set<Integer> parentAdresses, FeatureStructure featureValue) {
		...
		if (fsArray.get(0) instanceof Annotation) {
			...
			for (FeatureStructure featureStructure : fsArray) {
				AnnotationDto featureAnnotationDto = this.buildAnnotationDtoRecursive(jCas, (Annotation) featureStructure, parentAdresses);
	...
	private Object buildFeatureValueFromComplexType(JCas jCas, Annotation annotation, Feature feature, Set<Integer> parentAdresses) {
